...
echo "In this task you will configure quotas for API objects including PersistentVolumeClaims and Services.";
echo ""; sleep 3s

echo "A quota restricts the number of objects of a particular type that can be created in a namespace.";
echo ""; sleep 3s

echo "You specify quotas in a ResourceQuota object";
echo ""; sleep 3s

echo "Create a namespace called quota-object-example so that resources you create in this Task are isolated from the rest of your cluster";

SUCCESS=false;
while ! $SUCCESS; do
    if [ $($KUBECTL_CMD get namespaces --field-selector=metadata.name=quota-object-example -o custom-columns=NAME:.metadata.name --no-headers | wc -l) -eq 1 ]; then
    echo ""
...
echo "We will now create a ResourceQuota with a limit on the number of PersistentVolumeClaims.";
echo ""
echo "Generate the declarative configuration for the ResourceQuota with below configuration:"
echo ""
echo "ResourceQuota Name: object-quota-demo"
echo "Namespace: quota-object-example"
echo "hard.persistentvolumeclaims: 1"
echo "hard.services.loadbalancers: 2"
echo "hard.services.nodeports: 0"

SUCCESS=false;
while ! $SUCCESS; do
    if [ $($KUBECTL_CMD get resourcequota -n quota-object-example -o go-template --template '{{"\n"}}{{range .items}}{{if eq .metadata.name "object-quota-demo"}}{{if eq .spec.hard.persistentvolumeclaims "1"}}{{if eq (index .spec.hard "services.loadbalancers") "2"}}{{.}}{{end}}{{end}}{{end}}{{end}}{{"\n\n"}}' | wc | awk '{ print $2 }') -gt 0 ]; then
      echo "";
      echo "Great job creating the ResourceQuota!";
      sleep 5s
      SUCCESS=true;
    else
      sleep 5s;
